//! Dependency-tree renderer for `changepacks check --tree`.
//!
//! Extracted verbatim from `commands/check.rs` so the command handler stays a
//! thin argument/dispatch layer and the tree renderer — its frame enum, line
//! cache and dependency resolution — lives in one focused module. Nothing here
...
pub(super) fn version_display_with_update(
    project: &Project,
    repo_root_path: &Path,
    update_map: &HashMap<PathBuf, (UpdateType, Vec<ChangePackResultLog>)>,
) -> Result<String> {
